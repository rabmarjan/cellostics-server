...
from dauth.models import Token
...
def auth(request):
    ...
    tokenkey = request.data.get('token', None)
    username = request.data.get('user_id', None)
    token = Token.objects.get(user=int(username))
    # print(tokenkey, username, token.token)
...
class AccountList(generics.ListCreateAPIView):
    ...
    queryset = Account.objects.raw('select * from app_account')
...
class CustomBillCollection(APIView):
    def post(self, request, *args, **kwargs):
        #auth(request)
        tokenkey = request.data.get('api_key', None)
        username = request.data.get('user_id', None)
        try:
           token = Token.objects.get(user=int(username))
        except:
            return JsonResponse({"status": "Please Provide valid user id"})
        if token.token == tokenkey:
            pass
        else:
            return JsonResponse({"detail": "Authentication credentials were not provided."}, status=401)
        customlist =  BillCollection.objects.filter()
        serializer = BillCollectionSerializer(customlist, many=True)
...
class Custom_list(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        customlist = CustomList.objects.raw(customList(userid))
        serializer = CustomListSerializer(customlist, many=True)
        return Response(serializer.data)

# Don`t delete below comented lines
